...
TABLE_CONFIG = {
    ...
    'person': {
        'table': 'persons',
        'headers': ['Part', '공정', '부서', '폐기자'],
        'columns': ['part', 'process', 'department', 'name'],
    },
    '1Part_TMNO': {
        'table': 'tmnos',
...
def init_db():
    ...
    cur.execute('''
        CREATE TABLE IF NOT EXISTS persons (
            ...
            name TEXT NOT NULL,
            UNIQUE(part, process, department, name)
        )
    ''')
...
def migrate_from_excel():
    ...
    for row in wb['person'].iter_rows(min_row=2, values_only=True):
        if row[3]:
            cur.execute("INSERT OR IGNORE INTO persons (part, process, department, name) VALUES (?, ?, ?, ?)",
                       (str(row[0] or ''), str(row[1] or ''), str(row[2] or ''), str(row[3])))
...
def get_persons():
    ...
    conn = get_db()
    query = "SELECT name FROM persons WHERE 1=1"
    params = []
...
def get_tmnos():
    ...
    if col:
        rows = conn.execute(
            f"SELECT tmno FROM tmnos WHERE part_type = ? AND LOWER({col}) = 'y' ORDER BY tmno",
            (part,)
        ).fetchall()
    else:
        rows = conn.execute(
    ...
    if sheet_name == 'Data':
        ...
        try:
            conn.execute("""
                ...
                scrap_reason=?, quantity=?, weight=?, remark=? WHERE id=?
            """, data + [row_id])
            conn.commit()
...
def get_scrap_records():
    ...
    rows = conn.execute("""
        SELECT id, unique_id, date, part, department, process, machine, person,
               tmno, product_name, scrap_reason, quantity, weight, remark
        FROM scrap_data ORDER BY id DESC
    """).fetchall()
    conn.close()
    ...
    return jsonify({'success': True, 'headers': headers, 'rows': result_rows})
