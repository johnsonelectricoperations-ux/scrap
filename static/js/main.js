...
// render list of detailed reasons under 기타폐기
...
// render main categories: 공정불량, 셋팅불량, 기타폐기
function renderMainReasonButtons() {
  const container = document.getElementById('reasonList');
  container.innerHTML = '';

  const mainGrid = document.createElement('div');
  mainGrid.className = 'reason-main-grid';

  const mainCard = document.createElement('div');
  mainCard.className = 'reason-card';
  const mainCardBtns = document.createElement('div');
  mainCardBtns.className = 'reason-card-buttons';
  const btn1 = createSelectButton('공정불량', () => { selectReason('공정불량'); });
  const btn2 = createSelectButton('셋팅불량', () => { selectReason('셋팅불량'); });
  mainCardBtns.appendChild(btn1);
  mainCardBtns.appendChild(btn2);
  const mainDesc = document.createElement('div');
  mainDesc.className = 'reason-card-desc';
  mainDesc.textContent = '공정불량과 셋팅불량은 작업일보 또는 선별일지에 기록한 불량품을 폐기할 때 사용합니다.';
  mainCard.appendChild(mainCardBtns);
  mainCard.appendChild(mainDesc);

  const etcCard = document.createElement('div');
  etcCard.className = 'reason-card';
  const etcCardBtns = document.createElement('div');
  etcCardBtns.className = 'reason-card-buttons';
  const btn3 = createSelectButton('기타폐기', () => { renderEtcReasonButtons(); });
  etcCardBtns.appendChild(btn3);
  const etcDesc = document.createElement('div');
  etcDesc.className = 'reason-card-desc';
  etcDesc.textContent = '기록되지 않은 모든 폐기품을 폐기할 때 사용합니다.';
  etcCard.appendChild(etcCardBtns);
  etcCard.appendChild(etcDesc);

  mainGrid.appendChild(mainCard);
  mainGrid.appendChild(etcCard);
  container.appendChild(mainGrid);
}

function renderEtcReasonButtons() {
  // show back button
  const container = document.getElementById('reasonList');
  container.innerHTML = '';
  const backBtn = document.createElement('button');
  backBtn.className = 'btn btn-back-small';
  backBtn.textContent = '< 뒤로';
  backBtn.onclick = renderMainReasonButtons;
  container.appendChild(backBtn);

  renderReasonButtons(scrapReasons);
}
...
function editEntry(idx) {
  editIndex = idx;
  const entry = scrapEntries[idx];

  // 폼에 값 채우기
  document.getElementById('newMachine').value = entry.machine || '';
  document.getElementById('newReason').value = entry.reason;
  document.getElementById('newReasonBtn').textContent = entry.reason;
  document.getElementById('newReasonBtn').classList.add('selected');
  document.getElementById('newRemark').value = entry.remark || '';
  document.getElementById('newTmnoSearch').value = entry.tmno;
  document.getElementById('newTmno').value = entry.tmno;
  document.getElementById('newProductName').value = entry.productName;
  document.getElementById('newUnitWeight').value = entry.unitWeight;
  document.getElementById('newQuantity').value = entry.quantity;
  document.getElementById('newWeight').value = entry.weight;

  // 비고 표시 조정
  const remarkContainer = document.getElementById('remarkContainer');
  if (entry.reason.startsWith('기타')) {
    remarkContainer.style.display = 'block';
  } else {
    remarkContainer.style.display = 'none';
  }
  ...
  // 수량 입력 활성화
  enableQuantityInput(entry.tmno ? true : false);

  // 버튼 변경
  const btn = document.getElementById('addEntryBtn');
  btn.textContent = '수정 완료';
  btn.onclick = addOrUpdateEntry;
}
...
function addOrUpdateEntry() {
  ...
  if (editIndex >= 0) {
    scrapEntries[editIndex] = entry;
    editIndex = -1;
    showToast('항목이 수정되었습니다.', 'success');
  } else {
    scrapEntries.push(entry);
...
function updateEntriesTable() {
  ...
  scrapEntries.forEach((entry, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td></td>
    `;
    const editCell = tr.cells[8];
    const delCell = tr.cells[9];
...
function confirmSave() {
  ...
  // 일반 브라우저 확인 대화상자
  const proceed = window.confirm('입력한 값들을 모두 저장하겠습니까?');
  if (proceed) {
    saveAllData();
  }
}
...
async function showScrapRecords(btn, page) {
  ...
  if (!document.getElementById('scrapFilterPanel')) {
    ...
    const filterHtml = `
      <div id="scrapFilterPanel" class="scrap-filter-panel">
        <div class="scrap-filter-row">
          ...
          <div class="scrap-filter-group">
            <label>폐기자</label>
            <input type="text" id="scrapPersonFilter" class="scrap-filter-input" placeholder="이름">
          </div>
    ...
    if (scrapFilter.person) document.getElementById('scrapPersonFilter').value = scrapFilter.person;
  }
  ...
  if (scrapFilter.person) params += `&person=${encodeURIComponent(scrapFilter.person)}`;
...
function applyScrapFilter() {
  ...
  scrapFilter.person = document.getElementById('scrapPersonFilter').value.trim();
